# base on spdlog v2.2.2
function(spdlog_target)
    ...
    set(${module}_url https://github.com/gabime/spdlog.git)
    set(${module}_supported_version 1.11.0)
    set(${module}_supported_tag v1.11.0)
    version_selector(${module} ${module}_supported_version 1.11.0)
    ...
    function(fix_spdlog_target_name)
        get_target_property(i spdlog::spdlog IMPORTED)
        message(STATUS "IMPORTED(spdlog::spdlog):${i}")
        get_target_property(ic spdlog::spdlog IMPORTED_CONFIGURATIONS)
        message(STATUS "IMPORTED_CONFIGURATIONS:${ic}")
        get_target_property(ilr spdlog::spdlog IMPORTED_LOCATION_${ic})
        message(STATUS "IMPORTED_LOCATION_${ic}:${ilr}")
        get_target_property(isr spdlog::spdlog IMPORTED_SONAME_${ic})
        message(STATUS "IMPORTED_SONAME_${ic}:${isr}")
        get_target_property(icd spdlog::spdlog INTERFACE_COMPILE_DEFINITIONS)
        message(STATUS "INTERFACE_COMPILE_DEFINITIONS:${icd}")
        get_target_property(iid spdlog::spdlog INTERFACE_INCLUDE_DIRECTORIES)
        message(STATUS "INTERFACE_INCLUDE_CIRECTORIES:${iid}")
        get_target_property(ill spdlog::spdlog INTERFACE_LINK_LIBRARIES)
        message(STATUS "INTERFACE_LINK_LIBRARIES:${ill}")
    endfunction(fix_spdlog_target_name)

    if(${${module}_FOUND})
        message(DEBUG "GLOG FOUND(TEMP)")
        fix_spdlog_target_name()
        add_library(interface_lib${module} INTERFACE)
        target_link_libraries(interface_lib${module} INTERFACE spdlog::spdlog)
    else()
