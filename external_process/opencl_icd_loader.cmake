# base on opencl_icd_loader v2.2.2
function(opencl_icd_loader_target)
    ...
    set(module opencl_icd_loader)
    parser_the_arguments(${module} git)

    set(${module}_url https://github.com/KhronosGroup/OpenCL-ICD-Loader.git)
    ...
    function(fix_opencl_icd_loader_target_name)
        get_target_property(i opencl_icd_loader::opencl_icd_loader IMPORTED)
        message(STATUS "IMPORTED(opencl_icd_loader::opencl_icd_loader):${i}")
        get_target_property(ic opencl_icd_loader::opencl_icd_loader IMPORTED_CONFIGURATIONS)
        message(STATUS "IMPORTED_CONFIGURATIONS:${ic}")
        get_target_property(ilr opencl_icd_loader::opencl_icd_loader IMPORTED_LOCATION_${ic})
        message(STATUS "IMPORTED_LOCATION_${ic}:${ilr}")
        get_target_property(isr opencl_icd_loader::opencl_icd_loader IMPORTED_SONAME_${ic})
        message(STATUS "IMPORTED_SONAME_${ic}:${isr}")
        get_target_property(icd opencl_icd_loader::opencl_icd_loader INTERFACE_COMPILE_DEFINITIONS)
        message(STATUS "INTERFACE_COMPILE_DEFINITIONS:${icd}")
        get_target_property(iid opencl_icd_loader::opencl_icd_loader INTERFACE_INCLUDE_DIRECTORIES)
        message(STATUS "INTERFACE_INCLUDE_CIRECTORIES:${iid}")
        get_target_property(ill opencl_icd_loader::opencl_icd_loader INTERFACE_LINK_LIBRARIES)
        message(STATUS "INTERFACE_LINK_LIBRARIES:${ill}")
    endfunction(fix_opencl_icd_loader_target_name)

    if(${${module}_FOUND})
        message(DEBUG "GLOG FOUND(TEMP)")
        fix_opencl_icd_loader_target_name()
        add_library(interface_lib${module} INTERFACE)
        target_link_libraries(interface_lib${module} INTERFACE opencl_icd_loader::opencl_icd_loader)
    else()
        ...
        add_library(opencl_icd_loader::opencl_icd_loader UNKNOWN IMPORTED)
        set_target_properties(
            opencl_icd_loader::opencl_icd_loader
            PROPERTIES
...
endfunction(opencl_icd_loader_target)
