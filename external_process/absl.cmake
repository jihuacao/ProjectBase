...
function(fix_absl_target_name)
endfunction(fix_absl_target_name)

function(download_and_build_absl)
endfunction(download_and_build_absl)

if(${${module}_FOUND})
    message(STATUS "${module} Found for  ${${module}_version}")
    fix_absl_target_name()
    set(${module}_target_name absl::libabsl CACHE INTERNAL "module target name")
    set(${module}_target_name-lite absl::libabsl-lite CACHE INTERNAL "module target name")
else()
    ...
    if(_${module}_build_type STREQUAL "RELEASE")
        if(_${module}_build_shared)
            set(soname libabsl.so)
            set(soname-lite libabsl-lite.so)
            set(soname-c libprotoc.so)
            set(location ${external_install_path}/lib/libabsl.so)
            set(location-lite ${external_install_path}/lib/libabsl-lite.so)
            set(location-c ${external_install_path}/lib/libprotoc.so)
        else()
            set(soname libabsld.so)
            set(soname-lite libabsl-lited.so)
            set(soname-c libprotocd.so)
            set(location ${external_install_path}/lib/libabsld.so)
            set(location-lite ${external_install_path}/lib/libabsl-lited.so)
            set(location-c ${external_install_path}/lib/libprotocd.so)
        endif()
    else()
        if(_${module}_build_shared)
            set(soname libabsl.a)
            set(soname-lite libabsl-lite.a)
            set(soname-c libprotoc.a)
            set(location ${external_install_path}/lib/libabsl.a)
            set(location-lite ${external_install_path}/lib/libabsl-lite.a)
            set(location-c ${external_install_path}/lib/libprotoc.a)
        else()
            set(soname libabsld.a)
            set(soname-lite libabsl-lited.a)
            set(soname-c libprotocd.a)
            set(location ${external_install_path}/lib/libabsld.a)
            set(location-lite ${external_install_path}/lib/libabsl-lited.a)
...
#        #COMMAND ${CMAKE_COMMAND} -E chdir ${external_install_path} mkdir include/google/absl/cmake
#        COMMAND ${CMAKE_COMMAND} -E chdir ${external_download_dir}/${module} cp -rf ./cmake ${external_install_path}/include/google/absl
#    )
...
#第二套构建方法，使用cmake
    ...
    ExternalProject_Add(
        ...
        CMAKE_CACHE_ARGS
            ...
            -Dabsl_WITH_ZLIB:BOOL=ON
            -Dabsl_BUILD_SHARED_LIBS:BOOL=${_${module}_build_shared}
            -Dabsl_BUILD_EXAMPLES:BOOL=ON
            -Dabsl_BUILD_TESTS:BOOL=OFF
            -Dabsl_INSTALL_EXAMPLES:BOOl=OFF
