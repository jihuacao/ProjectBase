...
namespace ProjectBase{
    namespace Tree{
		...
		template<class T>
		inline duplex_tree_node<T>::duplex_tree_node()
			: _tree_node<T>(), _parent(0), _first_child(0), _last_child(0), _prev_sibling(0), _next_sibling(0)
			{
			}

		template<class T>
		inline duplex_tree_node<T>::duplex_tree_node(const T& val)
			: _tree_node<T>(val), _parent(0), _first_child(0), _last_child(0), _prev_sibling(0), _next_sibling(0){
		}

		template<class T>
		inline duplex_tree_node<T>::duplex_tree_node(T&& val)
			: _tree_node<T>(val), _parent(0), _first_child(0), _last_child(0), _prev_sibling(0), _next_sibling(0){
		}

		template<class T>
		inline _tree_node<T>* duplex_tree_node<T>::pre() const{
			return _prev_sibling;
		}

		template<class T> 
		inline _tree_node<T>* duplex_tree_node<T>::next() const{
		...
		template<class T> 
		inline _tree_node<T>* duplex_tree_node<T>::last_child() const{
			return _last_child;
		}

		template<class T> 
		inline _tree_node<T>* duplex_tree_node<T>::parent() const{
			return _parent;
		}
		 ...
		 * \brief brief
		 * \note note
		 * \param[in] Node 模板类型的Node类型指定
		 * \author none
		 * \since version
		 * */
		template <typename Node, typename tree_nodeallocator = std::allocator<duplex_tree_node<typename Node::value_type> > >
		class tree {
			protected:
			...
			public:
				...
				tree(const typename Node::value_type&);                                 // constructor setting given element as head
				tree(const iterator_base&);
				tree(const tree<Node, tree_nodeallocator>&);      // copy constructor
				tree(tree<Node, tree_nodeallocator>&&);           // move constructor
				~tree();
				tree<Node,tree_nodeallocator>& operator=(const tree<Node, tree_nodeallocator>&);   // copy assignment
				tree<Node,tree_nodeallocator>& operator=(tree<Node, tree_nodeallocator>&&);        // move assignment

		      /// Base class for iterators, only pointers stored, no traversal logic.
		#ifdef __SGI_Stypename Node::value_typeL_PORtypename Node::value_type
				class iterator_base : public stlport::bidirectional_iterator<typename Node::value_type, ptrdiff_t> {
		#else
				class iterator_base {
		#endif
					public:
				...
				/// typename Node::value_typehe default iterator types throughout the tree class.
				...
				/// typename Node::value_typehe first integer in path_t is the number of steps you need to go 'right' in the sibling
				...
				/// Return iterator to the parent of a node. typename Node::value_typehrows a `navigation_error` if the node
				...
				template<typename iter> iter append_child(iter position, const typename Node::value_type& x);
				template<typename iter> iter append_child(iter position, typename Node::value_type&& x);
				template<typename iter> iter prepend_child(iter position, const typename Node::value_type& x);
				template<typename iter> iter prepend_child(iter position, typename Node::value_type&& x);
				...
				pre_order_iterator set_head(const typename Node::value_type& x);
				pre_order_iterator set_head(typename Node::value_type&& x);
				/// Insert node as previous sibling of node pointed to by position.
				template<typename iter> iter insert(iter position, const typename Node::value_type& x);
				template<typename iter> iter insert(iter position, typename Node::value_type&& x);
				/// Specialisation of previous member.
				sibling_iterator insert(sibling_iterator position, const typename Node::value_type& x);
				...
				template<typename iter> iter insert_after(iter position, const typename Node::value_type& x);
				template<typename iter> iter insert_after(iter position, typename Node::value_type&& x);
				...
				template<typename iter> iter replace(iter position, const typename Node::value_type& x);
				...
				template<typename iter> iter wrap(iter position, const typename Node::value_type& x);
				/// Replace the range of sibling nodes (plus subtrees), making these children of the new node.
				template<typename iter> iter wrap(iter from, iter to, const typename Node::value_type& x);
				...
				/// Exchange two nodes (plus subtrees). typename Node::value_typehe iterators will remain valid and keep 
				...
				/// Count the number of siblings (left and right) of node at iterator. typename Node::value_typeotal nodes at this level is +1.
				...
				class iterator_base_less {
					public:
						bool operator()(const typename tree<Node, tree_nodeallocator>::iterator_base& one,
											 const typename tree<Node, tree_nodeallocator>::iterator_base& two) const
							{
			...
			private:
				...
				void copy_(const tree<Node, tree_nodeallocator>& other);
		...
		//template <typename Node, typename tree_nodeallocator>
		//class iterator_base_less {
		//	public:
		//		bool operator()(const typename tree<Node, tree_nodeallocator>::iterator_base& one,
		//						  const typename tree<Node, tree_nodeallocator>::iterator_base& two) const
		...
		// template <typename Node, typename tree_nodeallocator>
		// bool operator<(const typename tree<Node, tree_nodeallocator>::iterator& one,
		// 					const typename tree<Node, tree_nodeallocator>::iterator& two)
		// 	{
		...
		// template <typename Node, typename tree_nodeallocator>
		// bool operator>(const typename tree<Node, tree_nodeallocator>::iterator_base& one,
		// 					const typename tree<Node, tree_nodeallocator>::iterator_base& two)
		// 	{
		// 	txtout << "operator> " << one.node < two.node << std::endl;
		// 	if(one.node > two.node) return true;
		// 	return false;
		...
		// typename Node::value_typeree

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::tree() 
			{
			head_initialise_();
			}

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::tree(const typename Node::value_type& x) 
			{
			head_initialise_();
			set_head(x);
			}
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::tree(tree<Node, tree_nodeallocator>&& x) {
			head_initialise_();
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::tree(const iterator_base& other) {
			head_initialise_();
			set_head((*other));
			replace(begin(), other);
		}
		...
		template <typename Node, typename tree_nodeallocator>
		void tree<Node, tree_nodeallocator>::head_initialise_() {
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node,tree_nodeallocator>& tree<Node, tree_nodeallocator>::operator=(const tree<Node, tree_nodeallocator>& other)
			{
			if(this != &other)
				copy_(other);
			return *this;
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node,tree_nodeallocator>& tree<Node, tree_nodeallocator>::operator=(tree<Node, tree_nodeallocator>&& x)
			{
			if(this != &x) {
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::tree(const tree<Node, tree_nodeallocator>& other)
			{
			head_initialise_();
			copy_(other);
			}
		...
		template <typename Node, typename tree_nodeallocator>
		void tree<Node, tree_nodeallocator>::copy_(const tree<Node, tree_nodeallocator>& other) 
			{
			clear();
		...
		template <typename Node, typename tree_nodeallocator>
		void tree<Node, tree_nodeallocator>::clear()
			{
			if(head)
		...
		template<typename Node, typename tree_nodeallocator> 
		void tree<Node, tree_nodeallocator>::erase_children(const iterator_base& it)
			{
		...
		template<typename Node, typename tree_nodeallocator> 
		void tree<Node, tree_nodeallocator>::erase_right_siblings(const iterator_base& it)
			{
			if(it.node==0) return;
		...
		template<typename Node, typename tree_nodeallocator> 
		void tree<Node, tree_nodeallocator>::erase_left_siblings(const iterator_base& it)
			{
			if(it.node==0) return;
		...
		template<typename Node, typename tree_nodeallocator> 
		template<class iter>
		iter tree<Node, tree_nodeallocator>::erase(iter it)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::pre_order_iterator tree<Node, tree_nodeallocator>::begin() const
			{
			return pre_order_iterator(head->_next_sibling);
			}

		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::pre_order_iterator tree<Node, tree_nodeallocator>::end() const
			{
			return pre_order_iterator(feet);
			}

		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::breadth_first_queued_iterator tree<Node, tree_nodeallocator>::begin_breadth_first() const
			{
			return breadth_first_queued_iterator(head->_next_sibling);
			}

		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::breadth_first_queued_iterator tree<Node, tree_nodeallocator>::end_breadth_first() const
			{
			return breadth_first_queued_iterator();
			}

		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::post_order_iterator tree<Node, tree_nodeallocator>::begin_post() const
			{
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::post_order_iterator tree<Node, tree_nodeallocator>::end_post() const
			{
			return post_order_iterator(feet);
			}

		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::fixed_depth_iterator tree<Node, tree_nodeallocator>::begin_fixed(const iterator_base& pos, unsigned int dp, bool walk_back) const
			{
			typename tree<Node, tree_nodeallocator>::fixed_depth_iterator ret;
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::fixed_depth_iterator tree<Node, tree_nodeallocator>::end_fixed(const iterator_base& pos, unsigned int dp) const
			{
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::sibling_iterator tree<Node, tree_nodeallocator>::begin(const iterator_base& pos) 
			{
			assert(pos.node!=0);
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::sibling_iterator tree<Node, tree_nodeallocator>::end(const iterator_base& pos) 
			{
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::leaf_iterator tree<Node, tree_nodeallocator>::begin_leaf() const
		   {
		   tree_node *tmp=head->_next_sibling;
		   if(tmp!=feet) {
		      while(tmp->_first_child)
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::leaf_iterator tree<Node, tree_nodeallocator>::end_leaf() const
		   {
		   return leaf_iterator(feet);
		   }

		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::path_t tree<Node, tree_nodeallocator>::path_from_iterator(const iterator_base& iter, const iterator_base& top) const
			{
			path_t path;
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::iterator tree<Node, tree_nodeallocator>::iterator_from_path(const path_t& path, const iterator_base& top) const
			{
			iterator it=top;
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::leaf_iterator tree<Node, tree_nodeallocator>::begin_leaf(const iterator_base& top) const
		   {
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::leaf_iterator tree<Node, tree_nodeallocator>::end_leaf(const iterator_base& top) const
		   {
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter>
		iter tree<Node, tree_nodeallocator>::parent(iter position) 
			{
			if(position.node==0)
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter>
		iter tree<Node, tree_nodeallocator>::previous_sibling(iter position) 
			{
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter>
		iter tree<Node, tree_nodeallocator>::next_sibling(iter position) 
			{
			assert(position.node!=0);
			iter ret(position);
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter>
		iter tree<Node, tree_nodeallocator>::next_at_same_depth(iter position) const
			{
			// We make use of a temporary fixed_depth iterator to implement this.

			typename tree<Node, tree_nodeallocator>::fixed_depth_iterator tmp(position.node);
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter>
		iter tree<Node, tree_nodeallocator>::append_child(iter position)
		 	{
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter>
		iter tree<Node, tree_nodeallocator>::prepend_child(iter position)
		 	{
			assert(position.node!=head);
			assert(position.node!=feet);
			assert(position.node);
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::append_child(iter position, const typename Node::value_type& x)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::append_child(iter position, typename Node::value_type&& x)
			{
			assert(position.node!=head);
			assert(position.node!=feet);
			assert(position.node);
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::prepend_child(iter position, const typename Node::value_type& x)
			{
			assert(position.node!=head);
			assert(position.node!=feet);
			assert(position.node);
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::prepend_child(iter position, typename Node::value_type&& x)
			{
			assert(position.node!=head);
			assert(position.node!=feet);
			assert(position.node);
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::append_child(iter position, iter other)
			{
			assert(position.node!=head);
			assert(position.node!=feet);
			assert(position.node);
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::prepend_child(iter position, iter other)
			{
			assert(position.node!=head);
			assert(position.node!=feet);
			assert(position.node);
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::append_children(iter position, sibling_iterator from, sibling_iterator to)
			{
			assert(position.node!=head);
			assert(position.node!=feet);
			assert(position.node);
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::prepend_children(iter position, sibling_iterator from, sibling_iterator to)
			{
			assert(position.node!=head);
			assert(position.node!=feet);
			assert(position.node);
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::pre_order_iterator tree<Node, tree_nodeallocator>::set_head(const typename Node::value_type& x)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::pre_order_iterator tree<Node, tree_nodeallocator>::set_head(typename Node::value_type&& x)
			{
			assert(head->_next_sibling==feet);
			return insert(iterator(feet), x);
			}
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::insert(iter position, const typename Node::value_type& x)
			{
			if(position.node==0) {
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::insert(iter position, typename Node::value_type&& x)
			{
			if(position.node==0) {
				position.node=feet; // Backward compatibility: when calling insert on a null node,
				                    // insert before the feet.
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::sibling_iterator tree<Node, tree_nodeallocator>::insert(sibling_iterator position, const typename Node::value_type& x)
			{
			tree_node *tmp=std::allocator_traits<decltype(alloc_)>::allocate(alloc_, 1, 0);
			std::allocator_traits<decltype(alloc_)>::construct(alloc_, tmp, x);
			tmp->_first_child=0;
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::insert_after(iter position, const typename Node::value_type& x)
			{
			tree_node *tmp=std::allocator_traits<decltype(alloc_)>::allocate(alloc_, 1, 0);
			std::allocator_traits<decltype(alloc_)>::construct(alloc_, tmp, x);
			tmp->_first_child=0;
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::insert_after(iter position, typename Node::value_type&& x)
			{
			tree_node *tmp=std::allocator_traits<decltype(alloc_)>::allocate(alloc_, 1, 0);
			std::allocator_traits<decltype(alloc_)>::construct(alloc_, tmp);
			std::swap(tmp->data, x); // move semantics
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::insert_subtree(iter position, const iterator_base& subtree)
			{
			// insert dummy
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::insert_subtree_after(iter position, const iterator_base& subtree)
			{
			// insert dummy
		...
		// template <typename Node, typename tree_nodeallocator>
		// template <class iter>
		// iter tree<Node, tree_nodeallocator>::insert_subtree(sibling_iterator position, iter subtree)
		// 	{
		// 	// insert dummy
		// 	iter it(insert(position, value_type()));
		// 	// replace dummy with subtree
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::replace(iter position, const typename Node::value_type& x)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		template <class iter>
		iter tree<Node, tree_nodeallocator>::replace(iter position, const iterator_base& from)
			{
			assert(position.node!=head);
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::sibling_iterator tree<Node, tree_nodeallocator>::replace(
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter>
		iter tree<Node, tree_nodeallocator>::flatten(iter position)
			{
			if(position.node->_first_child==0)
				return position;
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter>
		iter tree<Node, tree_nodeallocator>::reparent(iter position, sibling_iterator begin, sibling_iterator end)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter> iter tree<Node, tree_nodeallocator>::reparent(iter position, iter from)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter> iter tree<Node, tree_nodeallocator>::wrap(iter position, const typename Node::value_type& x)
			{
			assert(position.node!=0);
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter> iter tree<Node, tree_nodeallocator>::wrap(iter from, iter to, const typename Node::value_type& x)
			{
			assert(from.node!=0);
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter> iter tree<Node, tree_nodeallocator>::move_after(iter target, iter source)
		   {
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter> iter tree<Node, tree_nodeallocator>::move_before(iter target, iter source)
		   {
		   tree_node *dst=target.node;
		   tree_node *src=source.node;
		   assert(dst);
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::sibling_iterator tree<Node, tree_nodeallocator>::move_before(sibling_iterator target, 
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter> iter tree<Node, tree_nodeallocator>::move_ontop(iter target, iter source)
			{
			tree_node *dst=target.node;
			tree_node *src=source.node;
			assert(dst);
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator> tree<Node, tree_nodeallocator>::move_out(iterator source)
			{
			tree ret;
		...
		template <typename Node, typename tree_nodeallocator>
		template<typename iter> iter tree<Node, tree_nodeallocator>::move_in(iter loc, tree& other)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		template<typename iter> iter tree<Node, tree_nodeallocator>::move_in_below(iter loc, tree& other)
			{
			if(other.head->_next_sibling==other.feet) return loc; // other tree is empty
		...
		template <typename Node, typename tree_nodeallocator>
		template<typename iter> iter tree<Node, tree_nodeallocator>::move_in_as_nth_child(iter loc, size_t n, tree& other)
			{
			if(other.head->_next_sibling==other.feet) return loc; // other tree is empty
		...
		template <typename Node, typename tree_nodeallocator>
		void tree<Node, tree_nodeallocator>::merge(sibling_iterator to1,   sibling_iterator to2,
		...
		template <typename Node, typename tree_nodeallocator>
		void tree<Node, tree_nodeallocator>::merge(iterator to, iterator from, bool duplicate_leaves)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		void tree<Node, tree_nodeallocator>::sort(sibling_iterator from, sibling_iterator to, bool deep)
			{
			std::less<typename Node::value_type> comp;
			sort(from, to, comp, deep);
			}

		template <typename Node, typename tree_nodeallocator>
		template <class StrictWeakOrdering>
		void tree<Node, tree_nodeallocator>::sort(sibling_iterator from, sibling_iterator to, 
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter>
		bool tree<Node, tree_nodeallocator>::equal(const iter& one_, const iter& two, const iter& three_) const
			{
			std::equal_to<typename Node::value_type> comp;
			return equal(one_, two, three_, comp);
			}

		template <typename Node, typename tree_nodeallocator>
		template <typename iter>
		bool tree<Node, tree_nodeallocator>::equal_subtree(const iter& one_, const iter& two_) const
			{
			std::equal_to<typename Node::value_type> comp;
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter, class BinaryPredicate>
		bool tree<Node, tree_nodeallocator>::equal(const iter& one_, const iter& two, const iter& three_, BinaryPredicate fun) const
			{
		...
		template <typename Node, typename tree_nodeallocator>
		template <typename iter, class BinaryPredicate>
		bool tree<Node, tree_nodeallocator>::equal_subtree(const iter& one_, const iter& two_, BinaryPredicate fun) const
			{
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator> tree<Node, tree_nodeallocator>::subtree(sibling_iterator from, sibling_iterator to) const
			{
		...
		template <typename Node, typename tree_nodeallocator>
		void tree<Node, tree_nodeallocator>::subtree(tree& tmp, sibling_iterator from, sibling_iterator to) const
			{
			assert(from!=to); // if from==to, the range is empty, hence no tree to return.
		...
		template <typename Node, typename tree_nodeallocator>
		size_t tree<Node, tree_nodeallocator>::size() const {
			size_t i=0;
		...
		template <typename Node, typename tree_nodeallocator>
		size_t tree<Node, tree_nodeallocator>::size(const iterator_base& top) const {
			size_t i=0;
		...
		template <typename Node, typename tree_nodeallocator>
		bool tree<Node, tree_nodeallocator>::empty() const
			{
			pre_order_iterator it=begin(), eit=end();
			return (it==eit);
			}
		...
		template <typename Node, typename tree_nodeallocator>
		int tree<Node, tree_nodeallocator>::depth(const iterator_base& it) {
		...
		template <typename Node, typename tree_nodeallocator>
		int tree<Node, tree_nodeallocator>::depth(const iterator_base& it, const iterator_base& root) 
			{
			tree_node* pos=it.node;
			assert(pos!=0);
			int ret=0;
		...
		template <typename Node, typename tree_nodeallocator>
		template <class Predicate>
		int tree<Node, tree_nodeallocator>::depth(const iterator_base& it, Predicate p) 
			{
			tree_node* pos=it.node;
			assert(pos!=0);
			int ret=0;
		...
		template <typename Node, typename tree_nodeallocator>
		template <class Predicate>
		int tree<Node, tree_nodeallocator>::distance(const iterator_base& top, const iterator_base& bottom, Predicate p) 
			{
		...
		template <typename Node, typename tree_nodeallocator>
		int tree<Node, tree_nodeallocator>::max_depth() const
			{
			int maxd=-1;
		...
		template <typename Node, typename tree_nodeallocator>
		int tree<Node, tree_nodeallocator>::max_depth(const iterator_base& pos) const{
			tree_node *tmp = pos.node;
		...
		template <typename Node, typename tree_nodeallocator>
		unsigned int tree<Node, tree_nodeallocator>::number_of_children(const iterator_base& it) 
		{
		...
		template <typename Node, typename tree_nodeallocator>
		unsigned int tree<Node, tree_nodeallocator>::number_of_siblings(const iterator_base& it) const
			{
			tree_node *pos=it.node;
			unsigned int ret=0;
			// count forward
		...
		template <typename Node, typename tree_nodeallocator>
		void tree<Node, tree_nodeallocator>::swap(sibling_iterator it)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		void tree<Node, tree_nodeallocator>::swap(iterator one, iterator two)
			{
		...
		// tree<Node, tree_nodeallocator>::iterator tree<Node, tree_nodeallocator>::find_subtree(
		...
		template <typename Node, typename tree_nodeallocator>
		bool tree<Node, tree_nodeallocator>::is_in_subtree(const iterator_base& it, const iterator_base& top) const
		   {
		...
		template <typename Node, typename tree_nodeallocator>
		bool tree<Node, tree_nodeallocator>::is_in_subtree(const iterator_base& it, const iterator_base& begin, 
																		 const iterator_base& end) const
			{
		...
		template <typename Node, typename tree_nodeallocator>
		bool tree<Node, tree_nodeallocator>::is_valid(const iterator_base& it) const
			{
		...
		template <typename Node, typename tree_nodeallocator>
		bool tree<Node, tree_nodeallocator>::is_head(const iterator_base& it) 
		  	{
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::iterator tree<Node, tree_nodeallocator>::lowest_common_ancestor(
		...
		template <typename Node, typename tree_nodeallocator>
		unsigned int tree<Node, tree_nodeallocator>::index(sibling_iterator it) const
			{
			unsigned int ind=0;
			if(it.node->_parent==0) {
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::sibling_iterator tree<Node, tree_nodeallocator>::sibling(const iterator_base& it, unsigned int num) const
		   {
		   tree_node *tmp;
		   if(it.node->_parent==0) {
		...
		template <typename Node, typename tree_nodeallocator>
		void tree<Node, tree_nodeallocator>::debug_verify_consistency() const
			{
			iterator it=begin();
			while(it!=end()) {
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::sibling_iterator tree<Node, tree_nodeallocator>::child(const iterator_base& it, unsigned int num) 
			{
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::iterator_base::iterator_base()
			: node(0), skip_current_children_(false)
			{
			}

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::iterator_base::iterator_base(tree_node *tn)
			: node(tn), skip_current_children_(false)
			{
			}

		template <typename Node, typename tree_nodeallocator>
		typename Node::value_type& tree<Node, tree_nodeallocator>::iterator_base::operator*() const
			{
			return node->data;
			}
		...
		template <typename Node, typename tree_nodeallocator>
		bool tree<Node, tree_nodeallocator>::post_order_iterator::operator!=(const post_order_iterator& other) const
			{
		...
		template <typename Node, typename tree_nodeallocator>
		bool tree<Node, tree_nodeallocator>::pre_order_iterator::operator!=(const pre_order_iterator& other) const
			{
			if(other.node!=this->node) return true;
			else return false;
			}
		...
		template <typename Node, typename tree_nodeallocator>
		bool tree<Node, tree_nodeallocator>::sibling_iterator::operator!=(const sibling_iterator& other) const
			{
			if(other.node!=this->node) return true;
			else return false;
			}
		...
		template <typename Node, typename tree_nodeallocator>
		bool tree<Node, tree_nodeallocator>::leaf_iterator::operator!=(const leaf_iterator& other) const
		   {
		   if(other.node!=this->node) return true;
		   else return false;
		   }
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::sibling_iterator tree<Node, tree_nodeallocator>::iterator_base::begin() const
			{
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::sibling_iterator tree<Node, tree_nodeallocator>::iterator_base::end() const
			{
			sibling_iterator ret(0);
			ret.parent_=node;
			return ret;
		...
		template <typename Node, typename tree_nodeallocator>
		void tree<Node, tree_nodeallocator>::iterator_base::skip_children()
			{
			skip_current_children_=true;
			}

		template <typename Node, typename tree_nodeallocator>
		void tree<Node, tree_nodeallocator>::iterator_base::skip_children(bool skip)
		   {
		   skip_current_children_=skip;
		   }

		template <typename Node, typename tree_nodeallocator>
		unsigned int tree<Node, tree_nodeallocator>::iterator_base::number_of_children() const
			{
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::pre_order_iterator::pre_order_iterator() 
			: iterator_base(0)
			{
			}

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::pre_order_iterator::pre_order_iterator(tree_node *tn)
			: iterator_base(tn)
			{
			}

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::pre_order_iterator::pre_order_iterator(const iterator_base &other)
			: iterator_base(other.node)
			{
			}

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::pre_order_iterator::pre_order_iterator(const sibling_iterator& other)
			: iterator_base(other.node)
			{
			if(this->node==0) {
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::pre_order_iterator& tree<Node, tree_nodeallocator>::pre_order_iterator::operator++() {
			assert(this->node!=0);
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::pre_order_iterator tree<Node, tree_nodeallocator>::pre_order_iterator::operator++(int)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::pre_order_iterator& tree<Node, tree_nodeallocator>::pre_order_iterator::next_skip_children() {
			(*this).skip_children();
			(*this)++;
			return *this;
		}
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::pre_order_iterator& tree<Node, tree_nodeallocator>::pre_order_iterator::operator+=(unsigned int num)
			{
			while(num>0) {
				++(*this);
				--num;
		...
		// Post-order iterator
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::post_order_iterator::post_order_iterator() 
			: iterator_base(0)
			{
			}

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::post_order_iterator::post_order_iterator(tree_node *tn)
			: iterator_base(tn)
			{
			}

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::post_order_iterator::post_order_iterator(const iterator_base &other)
			: iterator_base(other.node)
			{
			}

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::post_order_iterator::post_order_iterator(const sibling_iterator& other)
			: iterator_base(other.node)
			{
			if(this->node==0) {
				if(other.range_last()!=0)
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::post_order_iterator& tree<Node, tree_nodeallocator>::post_order_iterator::operator++(){
			assert(this->node != 0);
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::post_order_iterator tree<Node, tree_nodeallocator>::post_order_iterator::operator++(int)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::post_order_iterator& tree<Node, tree_nodeallocator>::post_order_iterator::operator+=(unsigned int num)
			{
			while(num>0) {
				++(*this);
				--num;
		...
		template <typename Node, typename tree_nodeallocator>
		void tree<Node, tree_nodeallocator>::post_order_iterator::descend_all()
			{
			assert(this->node!=0);
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::breadth_first_queued_iterator::breadth_first_queued_iterator()
			: iterator_base()
			{
			}

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::breadth_first_queued_iterator::breadth_first_queued_iterator(tree_node *tn)
			: iterator_base(tn)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::breadth_first_queued_iterator::breadth_first_queued_iterator(const iterator_base& other)
			: iterator_base(other.node)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		bool tree<Node, tree_nodeallocator>::breadth_first_queued_iterator::operator!=(const breadth_first_queued_iterator& other) const
			{
			if(other.node!=this->node) return true;
			else return false;
			}
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::breadth_first_queued_iterator& tree<Node, tree_nodeallocator>::breadth_first_queued_iterator::operator++()
			{
			assert(this->node!=0);
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::breadth_first_queued_iterator tree<Node, tree_nodeallocator>::breadth_first_queued_iterator::operator++(int)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::breadth_first_queued_iterator& tree<Node, tree_nodeallocator>::breadth_first_queued_iterator::operator+=(unsigned int num)
			{
			while(num>0) {
				++(*this);
				--num;
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::fixed_depth_iterator::fixed_depth_iterator()
			: iterator_base()
			{
			}

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::fixed_depth_iterator::fixed_depth_iterator(tree_node *tn)
			: iterator_base(tn), top_node(0)
			{
			}

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::fixed_depth_iterator::fixed_depth_iterator(const iterator_base& other)
			: iterator_base(other.node), top_node(0)
			{
			}

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::fixed_depth_iterator::fixed_depth_iterator(const sibling_iterator& other)
			: iterator_base(other.node), top_node(0)
			{
			}

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::fixed_depth_iterator::fixed_depth_iterator(const fixed_depth_iterator& other)
			: iterator_base(other.node), top_node(other.top_node)
			{
			}

		template <typename Node, typename tree_nodeallocator>
		bool tree<Node, tree_nodeallocator>::fixed_depth_iterator::operator==(const fixed_depth_iterator& other) const
			{
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::fixed_depth_iterator& tree<Node, tree_nodeallocator>::fixed_depth_iterator::operator++()
			{
			assert(this->node!=0);
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::fixed_depth_iterator tree<Node, tree_nodeallocator>::fixed_depth_iterator::operator++(int)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::fixed_depth_iterator& tree<Node, tree_nodeallocator>::fixed_depth_iterator::operator-=(unsigned int num)
			{
			while(num>0) {
				--(*this);
				--(num);
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::sibling_iterator::sibling_iterator() 
			: iterator_base()
			{
			set_parent_();
			}
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::sibling_iterator::sibling_iterator(tree_node *tn)
			: iterator_base(tn)
			{
			set_parent_();
			}
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::sibling_iterator::sibling_iterator(const iterator_base& other)
			: iterator_base(other.node)
			{
			set_parent_();
			}
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::sibling_iterator::sibling_iterator(const sibling_iterator& other)
			: iterator_base(other), parent_(other.parent_)
			{
			}

		template <typename Node, typename tree_nodeallocator>
		void tree<Node, tree_nodeallocator>::sibling_iterator::set_parent_()
			{
			parent_=0;
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::sibling_iterator& tree<Node, tree_nodeallocator>::sibling_iterator::operator++()
			{
			if(this->node)
				this->node=this->node->_next_sibling;
			return *this;
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::sibling_iterator tree<Node, tree_nodeallocator>::sibling_iterator::operator++(int)
			{
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::sibling_iterator& tree<Node, tree_nodeallocator>::sibling_iterator::operator+=(unsigned int num)
			{
			while(num>0) {
				++(*this);
				--num;
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::tree_node *tree<Node, tree_nodeallocator>::sibling_iterator::range_first() const
			{
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::tree_node *tree<Node, tree_nodeallocator>::sibling_iterator::range_last() const
			{
		...
		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::leaf_iterator::leaf_iterator() 
		   : iterator_base(0), top_node(0)
		   {
		   }

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::leaf_iterator::leaf_iterator(tree_node *tn, tree_node *top)
		   : iterator_base(tn), top_node(top)
		   {
		   }

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::leaf_iterator::leaf_iterator(const iterator_base &other)
		   : iterator_base(other.node), top_node(0)
		   {
		   }

		template <typename Node, typename tree_nodeallocator>
		tree<Node, tree_nodeallocator>::leaf_iterator::leaf_iterator(const sibling_iterator& other)
		   : iterator_base(other.node), top_node(0)
		   {
		   if(this->node==0) {
		      if(other.range_last()!=0)
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::leaf_iterator& tree<Node, tree_nodeallocator>::leaf_iterator::operator++(){
			assert(this->node!=0);
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::leaf_iterator tree<Node, tree_nodeallocator>::leaf_iterator::operator++(int){
		...
		template <typename Node, typename tree_nodeallocator>
		typename tree<Node, tree_nodeallocator>::leaf_iterator& tree<Node, tree_nodeallocator>::leaf_iterator::operator+=(unsigned int num){
		   while(num>0) {
		      ++(*this);
		      --num;
		      }
