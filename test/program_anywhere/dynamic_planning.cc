...
int k_th_smallest_path(int n, int h, int v, int k){
    // 假设选h，计算amount，如果amount > k，则继续
    // 假设选v，计算amount，如果amount < k, 则继续
}
std::string kthSmallestPath(std::vector<int>& destination, int k) {
    int h_lower_than_w = destination[0] < destination[1];
    int* p = (int*)malloc((destination[0] + destination[1] + 2) * ((h_lower_than_w ? destination[0] : destination[1]) + 1) * sizeof(int));
    k_th_smallest_path(destination[0] + destination[1] + 2, );
    return "";
}

TEST(OneSixFourThree, signle_test){
